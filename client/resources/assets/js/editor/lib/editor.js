(function () {
    ...
    function Editor() {
        ...
        var getNode = function () {
            var node, selection, range, root;
            root = properties.el.find(".section-inner")[0];
        ...
        var handleSmartList = function ($item, e) {
            ...
            chars = Utils.getCharacterPrecedingCaret(getNode());
        ...
        var handleListLineBreak = function ($li, e) {
            ...
            if ($list.children().length === 1 && $li.text() === "") {
                ...
                } else if ($li.prev().length !== 0 && $li.prev().text() === "" && Utils.getCharacterPrecedingCaret(getNode()) === "") {
                    e.preventDefault();
                    content = $li.html();
                    $list.after($paragraph);
                    $li.prev().remove();
        ...
        var handleNullAnchor = function () {
            ...
            if (sel.isCollapsed && sel.rangeCount > 0) {
                ...
                if (prev.hasClass("graf") || prev.hasClass("graf--mixtapeEmbed")) {
                    ...
                    node.remove();
                    markAsSelected.call(this, getNode());
        ...
        var handleMouseUp = function () {
            var anchor_node = getNode();
            if (!anchor_node) {
                return;
            }
            this.prev_current_node = anchor_node;
            handleTextSelection.call(this, anchor_node);
        ...
        var handleKeyDown = function (e) {
            ...
            if (e.which === keys.BACKSPACE) {
                eventHandled = false;
                components.tooltip_view.hide();
                if (this.reachedTop && Utils.isFirstChar(getNode())) {
                    return false;
                }
                anchor_node = getNode();
                ...
                if (parent.hasClass("graf--li") && Utils.getCharacterPrecedingCaret(getNode()).length === 0) {
                    return handleListBackspace(parent, e);
                }
                if ($(anchor_node).hasClass("graf--p") && Utils.isFirstChar(getNode())) {
                ...
                if ($(anchor_node).hasClass("graf--mixtapeEmbed") || $(anchor_node).hasClass("graf--iframe")) {
                    if (!$(anchor_node).text().trim() || Utils.isFirstChar(getNode())) {
                ...
                if ($(anchor_node).prev().hasClass("graf--mixtapeEmbed")) {
                    if (Utils.isFirstChar(getNode()) && !!$(anchor_node).text().trim()) {
                        return false;
                    }
                }
            }
            ...
            if (typeof anchor_node === "undefined" && $(".is-selected").hasClass("is-mediaFocused")) {
        ...
        var handleKeyUp = function (e) {
            ...
            if (e.which === keys.BACKSPACE) {
                ...
                if ($(anchor_node).hasClass("graf--first")) {
                    if (getSelectedText() === getNode().textContent) {
